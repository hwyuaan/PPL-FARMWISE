...
<body>
    <div class="flex flex-col justify-center items-center w-screen h-screen bg-slate-300">
        ...
        <div class="w-3/5 h-auto rounded-lg bg-white mb-3">
            ...
            <form class="flex flex-col pl-28 pr-28  pt-16 pb-16 w-items-center gap-y-2 " id="tambahdatapegawai"  action="{{ route('storepegawai') }}" method="post">
                @csrf
                @method('POST')
                <div class="text-4xl font-bold text-center text-green-400 mb-10">Tambah Data Pegawai</div>
                @if($errors->any())
            ...
            <script>
                document.getElementById('tambahdatapegawai').addEventListener('submit', function(event) {
                    var confirmation = confirm("Apakah anda akan menambahkan data pegawai ini?");
                    if (!confirmation) {
